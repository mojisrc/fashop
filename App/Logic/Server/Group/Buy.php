...
class Buy
{
    ...
    public function __construct(array $data)
    {
        ...
        //拼团活动
        $group = model('Group')->getGroupInfo(['id' => $this->getGroupId(), 'is_show' => 1, 'start_time' => ['elt', time()]]);
        if (!$group) {
        ...
        //拼团商品
        $group_goods = model('GroupGoods')->getGroupGoodsInfo(['group_id' => $this->getGroupId(), 'goods_id' => $this->getGoodsId(), 'goods_sku_id' => $this->getGoodsSkuId()], '', '*');
        if (!$group_goods) {
        ...
        //拼团订单
        $group_order_num = model('Order')->getOrderInfo(['user_id' => $this->getUserId(), 'group_id' => $this->getGroupId(), 'group_state' => ['in', '0,1,2']], '', 'COUNT(id) AS group_num', []);
        ...
        if ($this->getBuyWay() == 'join_group') {
            $captain_group_order = model('Order')->getOrderInfo(['id' => $this->getOrderId(), 'group_id' => $this->getGroupId(), 'group_state' => 1], '', '*', []);
    ...
    private function updateGoodsStorageNum(): void
    {
        ...
        $goods_result              = model('GoodsSku')->editGoodsSku($goods_sku_condition, $goods_sku_update_data);
        if (!$goods_result) {
            ...
            $goods_sku_result      = model('Goods')->editGoods($goods_condition, $goods_update_data);
            if (!$goods_sku_result) {
    ...
    public function goodsInfo(array $condition = [])
    {
        ...
        $info                      = model('GoodsSku')->alias('goods_sku')->join('goods', 'goods_sku.goods_id = goods.id', "LEFT")->join('freight', 'goods.freight_id = freight.id', "LEFT")->field($field)->where($condition)->group('goods_sku_id')->find();
