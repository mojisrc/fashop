...
class OrderRefundSearch
{
    ...
    public function buildCondition(): void
    {
        ...
        // 1 申请退款，等待商家确认 2同意申请，等待买家退货 3买家已发货，等待收货  4已收货，确认退款 5退款成功 6退款关闭 7同意退款，仅退款 8拒绝(驳回)
        ...
        if (!empty($this->refundState)) {
            switch ($this->refundState) {
                ...
                case 6:
                    ...
                    break;
                case 7:
                    $this->condition['refund_type']  = 1; //1为仅退款,2为退货退款)
                    $this->condition['handle_state'] = 20; //平台处理状态 默认0处理中(未处理) 10拒绝(驳回) 20同意 30成功(已完成)

                    break;
                case 8:
                    $this->condition['handle_state'] = 10; //拒绝(驳回)
                    break;
