...
class Group
{
	...
	public function haveGoods()
	{
		...
		$group_ids             = \App\Model\GroupGoods::init()->getGroupGoodsColumn( $condition, '', 'group_id' );
		...
		if( !$group_ids ){
			...
			$map['end_time']        = ['<', time()];
			$map['is_show']         = 0;
			$group_count            = \App\Model\Group::init()->getGroupCount( $map );
	...
	public function filteGoodsSku()
	{
		...
		$goods_skus = \App\Model\GoodsSku::init()->getGoodsSkuList( ['id' => ['in', $sku_ids]], 'id,price,stock,code,spec,weight', 'id asc', [1, 100] );
