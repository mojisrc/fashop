...
class BodyFormat
{
	...
	private function formatBodyItem( array $body ) : array
	{
		...
		break;
		case self::goods_group_type :
			$data = $this->formatGroupGoods( $body['options'], $body['data'] );
		break;
		default:
	...
	{
		$goods_title_rows = in_array( $options['goods_title_rows'], [1, 2] ) ? (int)$options['goods_title_rows'] : 1;
		if( count( $data ) > 0 ){
		...
		return [
			...
			'options' => [
				...
				'goods_title_rows' => $goods_title_rows,
			],
			'data'    => $data,
		];
	}
	...
	private function formatGroupGoods( array $options, array $data ) : array
	{
		$source_type       = in_array( $options['source_type'], ['auto', 'choose'] ) ? $options['source_type'] : 'auto';
		$goods_sort        = in_array( $options['goods_sort'], [1, 2, 3] ) ? $options['goods_sort'] : 1;
		$goods_display_num = ($options['goods_display_num'] >= 1 && $options['goods_display_num'] <= 12) ? (int)$options['goods_display_num'] : 6;
		$layout_style      = in_array( $options['layout_style'], [1, 2, 4, 5] ) ? (int)$options['layout_style'] : 1;
		$goods_title_rows  = in_array( $options['goods_title_rows'], [1, 2] ) ? (int)$options['goods_title_rows'] : 1;

		if( $source_type == 'choose' ){
			foreach( $data as $key => $item ){
				$data[$key] = [
					'id'    => (int)$item['id'],
					'img'   => (string)$item['img'],
		...
		return [
			'type'    => self::goods_group_type,
			'options' => [
				'source_type'       => $source_type,
				'goods_sort'        => $goods_sort,
				'goods_display_num' => $goods_display_num,
				'layout_style'      => $layout_style,
				'goods_title_rows'  => $goods_title_rows,
			],
			'data'    => ($source_type == 'choose') ? $data : [],
		];
	}
