...
class Goods
{
	...
	public function edit() : bool
	{
		...
		try{
			//查询是否涉及到拼团
			$logic_group = new \App\Biz\Group( ['goods_id' => $this->id] );
			$have_goods  = $logic_group->haveGoods();
			if( $have_goods ){
				unset( $data['price'] );
				unset( $data['stock'] );
				unset( $data['is_on_sale'] );
				unset( $data['sku_list'] );
				unset( $data['spec_list'] );
				unset( $data['sale_time'] );
			}
			...
			\App\Model\GoodsCategoryIds::init()->delGoodsCategoryIds( ['goods_id' => $this->id] );
			$addCategoryIds = [];
			foreach( $this->categoryIds as $key => $value ){
				$addCategoryIds[$key]['goods_id']    = $this->id;
				$addCategoryIds[$key]['category_id'] = $value;
	...
	public function del( $ids ) : bool
	{
		$goodsModel = new \App\Model\Goods;
		try{
			$goodsModel->delGoods( ['id' => ['in', $ids]] );
			\App\Model\GoodsSku::init()->delGoodsSku( ['goods_id' => ['in', $ids]] );
			\App\Model\GoodsCategoryIds::init()->delGoodsCategoryIds( ['goods_id' => ['in', $ids]] );
			$goodsModel->commit();
			return true;
		} catch( \Exception $e ){
			// 回滚事务
