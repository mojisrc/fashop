...
namespace App\Biz\Server\Group;
...
class Buy
{
    ...
    public function calculate(): \App\Biz\Server\Buy\CalculateResult
    {
        ...
        return new \App\Biz\Server\Buy\CalculateResult([
     ...
     * @throws \Exception
     */
    public function createOrder(): \App\Biz\Server\Buy\CreateOrderResult
    {
    ...
    private function updateGoodsStorageNum(): void
    {
        ...
        // sku数据
        $goods_sku_result = \App\Model\GoodsSku::init()->rawQuery("UPDATE $table_goods_sku SET stock=stock-$goods_num,sale_num=sale_num+$goods_num,group_sale_num=group_sale_num+$goods_num WHERE id=$goods_sku_id;");

        if (!$goods_sku_result) {
            throw new \Exception('更新库存GoodsSku失败');
        } else {
            // goods数据
            $goods_result = \App\Model\Goods::init()->rawQuery("UPDATE $table_goods SET stock=stock-$goods_num,sale_num=sale_num+$goods_num,group_sale_num=group_sale_num+$goods_num WHERE id=$goods_id;");
            if (!$goods_result) {
     ...
     * @throws \Exception
     */
    public function updateDistribution(int $user_id, int $order_id)
    {
        ...
        if ($distribution_info) {
            ...
            $order_goods_result = $order_goods_model->editOrderGoods(['order_id' => $order_id, 'goods_id' => $goods_id, 'goods_sku_id' => $goods_sku_id], $order_goods_distribution);
            ...
            if ($order_distribution) {
                $order_result = $order_model->editOrder(['id' => $order_id], $order_distribution);
                if (!$order_result) {
