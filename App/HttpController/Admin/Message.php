...
class Message extends Admin
{
	...
	public function index()
	{
		...
		$type       = ['info_detail' => 2, 'order_send' => 3, 'coupon_send' => 4, 'order_refund' => 5];
		...
		if( $get['start'] && $get['start'] != '' && $get['end'] && $get['end'] != '' ){
			...
			$condition['create_time'] = [['>=', $get['start']], ['<=', $get['end']]];

		} elseif( $get['start'] && $get['start'] != '' ){
			$get['start']             = strtotime( $get['start'] );
			$condition['create_time'] = ['>=', $get['start']];

		} elseif( $get['end'] && $get['end'] != '' ){
			$get['end']               = strtotime( $get['end'] );
			$condition['create_time'] = ['<=', $get['end']];
		...
		if( $get['send_state'] != '' ){
			if( $get['send_state'] == 0 ){
				$condition['send_state'] = ['=', 0];
			} elseif( $get['send_state'] == 1 ){
				$condition['send_state'] = ['=', 1];
			}
		}
	...
	public function add()
	{
		if( $this->post ){
			...
			$post['user_ids'] = $post['user_ids'] ? array_unique( $post['user_ids'] ) : [];
	...
	public function messageTypeAdd()
	{
		if( $this->post ){
			$post = $this->post;
			\App\Model\MessageType::init()->addMessageType( $post );
	...
	public function messageTypeEdit()
	{
		if( $this->post ){
			$post = $this->post;
			\App\Model\MessageType::init()->editMessageType( ['id' => $post['id']], $post );
