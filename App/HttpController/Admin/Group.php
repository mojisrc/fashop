...
class Group extends Admin
{
    ...
    public function list()
    {
        ...
        $count       = $group_model->getGroupCount($condition);
        $list        = $group_model->getGroupList($condition, '*', 'id desc', $this->getPageLimit());
    ...
    public function add()
    {
        ...
        if ($error !== true) {
            ...
            $goods_info = $goods_model->getGoodsInfo(['is_on_sale' => 1, 'id' => $goods_id]);
            if (!$goods_info) {
                return $this->send(Code::param_error, [], '商品信息错误');
            }

            $goods_sku = $goods_sku_model->getGoodsSkuList(['goods_id' => $goods_id], '*', 'id desc', [1, 100]);
            if (!$goods_sku) {
                return $this->send(Code::param_error, [], '商品信息错误');
            }
            ...
            $group_info            = $group_model->where($condition_str)->getGroupInfo($condition);
            if ($group_info) {
                return $this->send(Code::param_error, [], '商品信息错误，该商品正在参加拼团');
            }
            ...
            $group_goods_result = $group_goods_model->addMultiGroupGoods($group_goods);
    ...
    public function edit()
    {
        ...
        if ($error !== true) {
            ...
            if ($group_goods_updata) {
                $group_goods_result = $group_goods_model->editMultiGroupGoods($group_goods_updata);
                if (!$group_goods_result) {
    ...
    public function selectableGoods()
    {
        ...
        $condition['is_show'] = 1;
        $condition_str        = "(start_time>$time) OR (start_time<=$time AND end_time>=$time)";
        $group_ids            = $group_model->where($condition_str)->getGroupColumn($condition);
