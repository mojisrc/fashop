...
class Group extends Server
{
	...
	public function info()
	{
		...
		if( $this->validator( $get, 'Server/Group.info' ) !== true ){
			...
			$goods_info          = \App\Model\Goods::init()->getGoodsInfo( ['id' => $goods_id] );
			$goods_info['skus']  = \App\Model\GoodsSku::init()->getGoodsSkuList( ['goods_id' => $goods_id], '*', 'id desc', [1, 100] );
	...
	public function groupingInfo()
	{
		...
		if( $this->validator( $get, 'Server/Group.groupingInfo' ) !== true ){
			...
			$order_goods_id = \App\Model\OrderGoods::init()->getOrderGoodsId( ['order_id' => $order_id, 'goods_id' => $goods_id] );
			if( !$order_goods_id ){
