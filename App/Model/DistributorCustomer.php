...
class DistributorCustomer extends Model
{
    ...
    {
        if( $page == '' ){
            return $this->join( 'distributor_user', 'distributor_customer.distributor_user_id = user.id', 'LEFT' )->join( 'user', 'distributor_customer.user_id = user.id', 'LEFT' )->where( $condition )->where( $condition_str )->order( $order )->field( $field )->group( $group )->select();

        } else{
            return $this->join( 'distributor_user', 'distributor_customer.distributor_user_id = user.id', 'LEFT' )->join( 'user', 'distributor_customer.user_id = user.id', 'LEFT' )->where( $condition )->where( $condition_str )->order( $order )->field( $field )->page( $page )->group( $group )->select();
    ...
    {
        if( $distinct == '' ){
            return $this->join( 'distributor_user', 'distributor_customer.distributor_user_id = user.id', 'LEFT' )->join( 'user', 'distributor_customer.user_id = user.id', 'LEFT' )->where( $condition )->where( $condition_str )->count();

        } else{
            return $this->join( 'distributor_user', 'distributor_customer.distributor_user_id = user.id', 'LEFT' )->join( 'user', 'distributor_customer.user_id = user.id', 'LEFT' )->where( $condition )->where( $condition_str )->count( "DISTINCT ".$distinct );
        }
    }
    ...
    {
        $data = $this->join( 'distributor_user', 'distributor_customer.distributor_user_id = user.id', 'LEFT' )->join( 'user', 'distributor_customer.user_id = user.id', 'LEFT' )->where( $condition )->where( $condition_str )->field( $field )->find();
        return $data;
    }
    ...
    {
        $data = $this->join( 'distributor_user', 'distributor_customer.distributor_user_id = user.id', 'LEFT' )->join( 'user', 'distributor_customer.user_id = user.id', 'LEFT' )->where( $condition )->where( $condition_str )->order($order)->field( $field )->find();
        return $data;
    }
    /**
     * 获取的id
