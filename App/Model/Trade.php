...
class Trade extends Model
{
	...
	{
		$goods_list = $this->table( '__ORDER_GOODS__' )->field( 'order_id,goods_num,goods_id' )->where( ['order_id' => $order_id] )->select(); //订单商品
		if( !empty( $goods_list ) && is_array( $goods_list ) ){
			...
			$state                = Order::init()->editOrder( $order_array, ['id' => $order_id] ); //更新订单
			if( $state ){
				$log_array['order_state'] = $order_array['state'];
				$state                    = OrderLog::init()->addOrderLog( $log_array );
			}
			return $state;
		}
		return false;
	 ...
	 * @param    array $log_array 订单记录信息
	 */
	public function editOrderFinnsh( $order_id, $log_array = [] )
	{
		$field           = 'id,user_id,user_name,id,sn,amount,payment_code,state';
		$order           = $this->table( '__ORDER__' )->field( $field )->where( ['id' => $order_id] )->find();
		$order_shipped   = $this->getOrderState( 'order_shipped' ); //订单状态30:已发货
		...
		if( $order['state'] == $order_shipped ){
			...
			$state                        = Order::init()->editOrder( $order_array, ['id' => $order_id] ); //更新订单状态为已收货
			$log_array['order_state']     = $order_array['state'];
			if( $state ){
				$state = OrderLog::init()->addOrderLog( $log_array );
			}
