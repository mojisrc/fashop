...
class userTest extends BaseTestCase
{
    ...
    {
        $response = self::$client->request( 'POST', "server/user/login", [
            'form_params' => [
                'login_type' => 'password',
                'username' => 'admin',
                'password' => '123456',
...
//            ]
    ...
    {
        $response = self::$client->request( 'POST', "server/user/register", [
            'form_params' => [
                'login_type' => 'password',
                'username' => 'admin',
                'password' => '123456',
...
//            ]
    ...
    {
        $response = self::$client->request( 'GET', "server/user/logout", [
            'headers' => ['access-token' => self::$accessToken]
        ]);
        $return_data = json_decode($response->getBody(), true);
        $this->assertEquals( self::$code::success, $return_data['code'], $response->getBody());
    ...
    {
        $response = self::$client->request( 'POST', "server/user/editPassword", [
            'form_params' => [
                'oldpassword' => '654321',
                'password' => '123456',
            ],
    ...
    {
        $response = self::$client->request( 'POST', "server/user/editPasswordByFind", [
            'form_params' => [
                'phone' => '18526459531',
                'password' => '123456',
                'verify_code' => '1234',
    ...
    {
        $response = self::$client->request( 'POST', "server/Verifycode/add", [
            'form_params' => [
                'behavior' => 'register',
                'channel_type' => 'sms',
                'phone' => '18526459531',
    ...
    {
        $response = self::$client->request( 'GET', "server/user/self", [
            'headers' => ['access-token' => self::$accessToken]
        ]);
        $return_data = json_decode($response->getBody(), true);
        $this->assertEquals( self::$code::success, $return_data['code'], $response->getBody());
    ...
    {
        $response = self::$client->request( 'POST', "server/user/editProfile", [
            'form_params' => [
                'province_id' => 1,
                'city_id' => 1,
                'area_id' => 1,
    ...
    {
        $response = self::$client->request( 'POST', "server/user/token", [
            'headers' => ['access-token' => self::$accessToken]
        ]);
        $return_data = json_decode($response->getBody(), true);
        $this->assertEquals( self::$code::success, $return_data['code'], $response->getBody());
    ...
    {
        $response = self::$client->request( 'POST', "server/user/bindPhone", [
            'form_params' => [
                'phone' => '18526459531',
                'password' => '123456',
                'verify_code' => '1234',
    ...
    {
        $response = self::$client->request( 'POST', "server/user/BindWechat", [
            'form_params' => [
                'wechat_openid' => '123456789',
                'wechat' => [
                    'openid' => 'openid',
    ...
    {
        $response = self::$client->request( 'POST', "server/user/unbindWechat", [
            'headers' => ['access-token' => self::$accessToken]
        ]);
        $return_data = json_decode($response->getBody(), true);
    ...
    {
        $response = self::$client->request( 'POST', "server/user/unbindPhone", [
            'headers' => ['access-token' => self::$accessToken]
        ]);
        $return_data = json_decode($response->getBody(), true);
        $this->assertEquals( self::$code::success, $return_data['code'], $response->getBody());
